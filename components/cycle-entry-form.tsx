...
type Entry = { seconds: string; excluded: boolean; exclusionReason: string };

export function CycleEntryForm({ models, lines, elements }: { models: Master[]; lines: Master[]; elements: Master[] }) {
  const [entries, setEntries] = useState<Entry[]>(Array.from({ length: 30 }, () => ({ seconds: "", excluded: false, exclusionReason: "" })));
  ...
  function pasteCycles(text: string) {
    ...
    setEntries(values.map((seconds) => ({ seconds: String(seconds), excluded: false, exclusionReason: "" })));
  }

  function update(index: number, patch: Partial<Entry>) { setEntries((current) => current.map((entry, i) => i === index ? { ...entry, ...patch } : entry)); }
  function addCycle() { if (entries.length < 100) setEntries((current) => [...current, { seconds: "", excluded: false, exclusionReason: "" }]); }
