...
export function YamazumiChart({ rows, scenarios, stationCodes }: { rows: Row[]; scenarios: Scenario[]; stationCodes: string[] }) {
  ...
  const series = buildYamazumiSeries(rows.map((row) => ({ stationCode: row.station_code, processCode: row.process_code, timeType: row.time_type, seconds: Number(row.standard_time_seconds) })), Number(scenario.takt_time_seconds), stationCodes);
  ...
  return <div className="yamazumi-chart">
    ...
    <div className={`yamazumi-guidance ${overloaded.length ? "yamazumi-guidance--warn" : ""}`}><strong>{overloaded.length ? `${overloaded.length} overloaded station${overloaded.length > 1 ? "s" : ""}` : "All stations within takt"}</strong><p>{overloaded.length ? overloaded.map((station) => `${station.code} exceeds takt by ${station.excessSeconds.toFixed(2)} sec`).join(" · ") + ". Review and rebalance the assigned work elements." : "No station cycle time exceeds the selected scenario takt."}</p></div>
  </div>;
}
